...
PolicyEngine projects household bus and coach fare spending to 2027 by English region outside London, family type, age and income quintile. Household fares are imputed from the LCFS and calibrated to DfT Annual Bus Statistics in `policyengine-uk-data`, then allocated to household members by `person_bus_fare_spending` in `policyengine-uk`, which applies an NTS-derived age profile. This repo contains only the reform modelling and the dashboard.
